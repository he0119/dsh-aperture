 ...
 * `src/client/index.tsx`）共享同一组端点名，谁写错一个字都不会有编译期报错，只会在浏览器里
...
describe('PANEL_INVOCATIONS 的端点身份', () => {
  it('id、服务名与命名空间按同一份文法拼出，且只声明一个直调', () => {
    ...
    for (const entry of PANEL_INVOCATIONS) {
      // 浏览器右半边（`src/client/index.tsx`）按同一份文法拼出 `${PACKAGE}#${PANEL}/${method}`，
      // 网关、注册表与客户端三方共用它。
  ...
  it('宿主侧描述符用 src-json 编解码器', () => {
    ...
    // 工厂是**浏览器那一半**（`src/client/index.tsx` 里的直通 CODEC）的事，两边不是同一份描述符。
    for (const entry of PANEL_INVOCATIONS) {
