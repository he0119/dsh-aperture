 ...
 *
 * 这一端是手工登记的：生成描述符的 Typert 生成器不随 DSH 发布，而两端（这里与
 * `src/client/remote.ts`）共享同一组端点名，谁写错一个字都不会有编译期报错，只会在浏览器里
 ...
 *
 * Host 服务只剩委托：`AperturePanelService` 的每个方法都必须原样把参数交给注入的
...
describe('PANEL_INVOCATIONS 的端点身份', () => {
  it('id、服务名与命名空间按同一份文法拼出，且只声明一个直调', () => {
    ...
    for (const entry of PANEL_INVOCATIONS) {
      // Web Client 端（`src/client/remote.ts`）按同一份文法拼出 `${PACKAGE}#${PANEL}/${method}`，
      // 网关、注册表与客户端三方共用它。
  ...
  it('宿主侧描述符用 src-json 编解码器', () => {
    // Host 端交接的是已经解析好的值，因此按 `src-json` 登记——strict 校验与 `create()`
    // 工厂是 **Web Client 端**（`src/client/remote.ts` 里的直通 CODEC）的事，两端不是同一份描述符。
    for (const entry of PANEL_INVOCATIONS) {
...
describe('PANEL_CONTRIBUTION', () => {
  it('是宿主面，带上同一份端点表，且不声明 schema 与 model', () => {
    ...
    assert.equal(PANEL_CONTRIBUTION.face, 'host', '这份描述符由 Host 端登记');
...
describe('AperturePanelService', () => {
  ...
  it('注册在命名空间这个名字下，Typert 绑定也指向它', () => {
    ...
    // Web Client 端注入的是 `remote.aperturePanel`：注册名、描述符里的 service/namespace 三者
