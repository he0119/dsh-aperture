 ...
 * 报告：配置页的状态段与模型清单。
 *
 * 这里是**结构化数据**，不是拼好的句子：配置页是双语的，把中文句子拼在 Host 端就等于让英文
 * 界面显示中文，而「哪个模型属于哪条路由」「这条事实是谁给的」本来就是数据。宿主只把发现过程
 * 决定了什么摆齐——哪个端点应答了、哪些模型进了哪条路由、哪个模型没有端点能服务、每条事实来自
 * 哪里、哪些字段是用户写下的覆盖——措辞与排版交给界面。
 *
...
export interface PanelModel {
   ...
   *
   * 这是「覆盖」的唯一判据，且只能从用户层读——官方也是这么定义的：看字段在不在用户层里，
   * 而不是拿值与默认值比。解析后的配置用不得：schema 会把没写的数组字段补成 `[]`，界面就会
   * 在一行什么都没写过的模型上挂一颗永远撤不掉的「已覆盖」。
   */
