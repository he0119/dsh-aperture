...
**地址与同步开关**是同一份官方设置表单里的两个字段：写着「已覆盖」时，右边那颗「恢复默认」把它从设置文件里删掉（与官方「插件配置」页同一对词、同一个位置），按「保存」只发真的改动过的那一项。逐模型的覆盖编辑器与发现报告见[使用](#使用)。没有卡片，也没有第二层折叠——分组靠标题与间距分开，能点开的只有模型行。

## 使用

界面在 **插件 → dsh-aperture**：在插件列表里点开本插件，配置页挂在包上，所以没有单独的「配置」按钮。页头的名字、图标与那句描述来自本包的 `locale/*.json` 与 `package.json` 的 `icon`，页面本身由插件页画出。地址、开关与逐模型参数在按下「保存」/「保存这一行」之前都只是草稿；模型那部分**按行来**，收起一行不算放弃，行上的「有未保存的改动」标签会一直挂着。
...
| 配置页 · 模型 | 一行一个模型：收起来是一行事实（路由、协议、容量、模态、推理，以及覆盖了哪几项），点开就是这一行的覆盖编辑器——显示名、协议、上下文容量、最大输出、清单别名各一个输入框，模态两个勾选框，推理一个三段开关（跟随发现 / 开 / 关）；每个字段下面那句注脚说着它现在的值来自哪里（「留空即用发现到的名字。来源：Aperture」），收起时完全不出现；右上角「立刻刷新」立刻重新发现并发布，并把这一轮的报告摆出来 |
...
**有些模型没出现在选择器里？** 看配置页上的「模型与路由」（「未服务」那一行）。只提供 Gemini 原生 `generateContent` 端点的模型接不进来——`llm-pi-ai` 只讲 OpenAI 兼容和 Anthropic Messages 两种协议。走错端点时 Aperture 会明确告诉你该用哪个：
