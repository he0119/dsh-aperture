...
| 位置 | 作用 |
| --- | --- |
| 标签页 · 实例卡 | 实例地址与同步开关；卡头的圆点说地址填了没有，右边的「撤销覆盖」把 `baseUrl` 放回继承 |
