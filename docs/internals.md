...
浏览器半边只注入 `slots` / `locale` / `remote` 三个服务。`baseUrl` 与 `sync` 这两项**设置**不自己
往返：槽位在 `view: 'page'` 时把插件页那份表单递进来（`form`，`ConfigPageForm`），界面读它的
`state`（生效值、用户层、版本号、这份文档收不收写入），写入交给 `form.mutate(ops, revision)`。
于是并发控制、冲突重读、只读判定都留在设置接缝自己手里，本插件不复制一套；写完仍然等一轮刷新
落地（见下），因为报告里的路由与模型事实来自最近一次刷新。「已覆盖」看的是字段在不在用户层里
（`state.user`），而不是值等不等于组合层，这两件事不是一回事。拿不到快照时——`state.status`
是 `unavailable`，或者插件页根本没递来 `form`——配置卡换成一句说明，报告照常显示：报告才是这一页
每次都有的东西，缺一份设置快照不该让整页停在「正在读取」。
...
- **发现结果与逐模型编辑走 Remote，设置不走。** 报告、立刻刷新与逐模型编辑挂在
  ...
  两个键，因此走页主递来的 `form`，不再借道 Remote。宿主侧仍然用
...
- **写完等一轮刷新落地才回答。** 配置页拿到回答就会重读报告，而报告里的路由与模型事实来自
  **最近一次刷新**（只有「已覆盖」与别名来自实时配置）。因此两条写路径在写入之后都
  `await runtime.refresh('配置变更')`——Remote 的 `edit` 在宿主侧做，地址与同步在浏览器半边做完
  `form.mutate` 之后做；不等它，界面重读到的还是旧配置算出来的那一份——地址换了、
...
**卡片是官方 `rowCard`**：`.5px` 的 `border-l4`、16px 圆角、`12px 14px` 内边距、底下不铺色。实例卡
与**路由卡**在页面上是同一层，共用同一条规则（`.dap-section, .dap-route`）；模型行是路由编辑区
**里面**的一层，用官方 `modelEntry` 的细框（10px 圆角、`10px 12px` 内边距）——两层的
...
的）——同一块浅色面上再套一块同色的面，边界根本看不见。两张卡各有各的担子——实例（地址、同步开关
与最近一次刷新）、路由与模型。

**「最近一次刷新」不配一张卡。** 它是一份只读诊断：一次刷新决定了什么、被什么触发。原先它自己
占一张卡，于是页面上出现了两个卡头、两枚状态点、两处折叠开关，而其中一张的内容只是四五行事实
——摘要不值得一张卡的卡头。现在它接在实例卡体里，同一块浅色面上切一条线（与模型参数面同一套
做法），排在「取消 / 保存」之后：表单与诊断的分界正是那排按钮。卡头那颗点于是改成说**整张卡**
——地址能不能用、上一轮成不成功，哪种出问题都变红，`title` 与 `aria-label` 点名是哪一种（颜色
不是唯一的说法）；刷新仍是这张卡卡头上的动作，与折叠开关并列。

**卡头自己就是折叠开关。** 箭头 + 身份那一段整体是一个按钮（官方分组头 `groupToggle`
...
`state-error-primary` 上色，语义换成「这个实例能不能用」（实例卡：地址可用、上一轮刷新没失败）、
「这一轮有没有把它写进 `llm-pi-ai`」（路由卡）与「有没有路由可用」（「未服务」那张卡）；
报告里根本没有同步结果时，路由卡就没有状态可说，索性不画。颜色不是唯一的说法：
...
**「已覆盖」与「恢复默认」照官方的词与位置。** 官方「插件配置」那页给这件事的词就是这两个
（`overridden` / `reset`），而且把它们做成**字段标签右边的一对**（`ValueField` 的 `head` 里一个
`badges`：一枚 Tag 加一颗纯文字按钮）。本插件原先把「已覆盖」挂在卡头上、把「撤销覆盖」当成卡头
的动作，于是标签离它描述的字段太远，看着像在说别的东西，而「撤销覆盖」这个自造词也没说出结果是
什么。现在两者都跟着字段走：`field()` 多收一个 `badges`，实例地址与同步开关各带一对，卡头只留
身份。语义仍然是官方的定义——**看字段在不在用户层里，而不是比较值**：写了个与默认相同的值，也
是覆盖（`describe()` 返回的 `user`）。「恢复默认」因此发的是一条 `unset` 路径操作——设置文档里
那个键被撤掉，缺省值重新生效；同步开关也能被单独撤掉，而「保存」只发真的改动过的那一项，
没动的项不会被顺手写进用户层。

**「恢复默认」写的是这一行的补丁，不是整条删除。** 报告里的 `overrideKeys` 写着这一行**确实**在
用户层里写过哪几项，按钮就把认得的那几项逐个 `null` 发出去，而不是丢掉 `aperture.models` 里整条
条目——后者会把界面根本不编辑的键（例如 `reasoningEfforts`）一起扔掉。反过来，只要有**一个**界面
不认识的键在名单里，就只能整条撤：只清认得的那几项，这一行在报告里仍然是「已覆盖」，那颗标签会
按不下去——用户报上来的正是这个现象。别名也在同一张名单里（写成 `alias`），撤的时候用空串，
因为它是 `modelAliases` 里的另一张表。
...
**最短**形式（`384000` → `384K`，而 `1048576` 不是整千，照原样写出来），失焦时再定形一次
（`100k` → `100K`）——只换写法不算改动，否则值没变却会往用户层里写一笔，那一行就凭空多一颗
「已覆盖」。两边就是官方那两个函数（`parseCapacity` / `formatCapacity`）：空串是「这一项不覆盖」，
读不出来的在本地挡下——`1G`、`1.5`、`100` 加个不在词汇里的后缀都不发端点，因为面板那一层只收
不小于 1 的整数，让它过去只会换来一次没必要的往返与一句后端的话。容量在只读那一行里仍写成
`1,048,576` 这种带千位分隔符的原文：那说的是**生效值**（来自哪个来源），不是你要填的那个数。
...
迟迟不见效」——同一个功能的两半正好分在两边（卡片、折叠、容量的 K/M 写法在浏览器半边；报告的
覆盖口径、写完等刷新的时序在宿主半边）。`lib/` 的 mtime 比 `src/` 旧就说明还没构建。
...
- **配置页只存在于 Web 界面**（插件页 + Typert 注册表）；没有它的部署里发现照常，只是没有可点按的界面。从非本机来源打开的页面拿不到宿主设置，配置页会把失败原因摆在页面上（而不是假装可编辑）；设置文档本身不接受写入时（表单快照的 `state.writable === false`），表单会置灰并说明原因；连快照都拿不到时（`state.status === 'unavailable'`）配置卡只剩一句说明，报告照常显示。
