...
- **发现结果也走 Remote。** 报告、立刻刷新、配置读写与逐模型编辑一样，都挂在
  `aperturePanel` 命名空间上（`src/remote.ts` 的宿主服务 + `src/panel.ts` 的五个端点）。发现
...
是覆盖（`describe()` 返回的 `user`）。`save` 因此两个参数都收 `null` 表示「恢复默认」（省略才是
「不碰」），同步开关也能被单独撤掉。
...
- **动作端点的一句结论仍是中文**：`PanelAction.summary` 由宿主半边写好（「已保存…」「已写入 2 条路由…」），因此英文界面里那一行也是中文。报告已经不走这条路（它是结构化数据），但这几个动作用的还是「宿主说一句话」的形态；要让它跟着语言走，得把 `summary` 换成「码 + 实参」再由界面渲染。
